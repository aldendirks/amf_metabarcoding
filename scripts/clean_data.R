...
# PROCESS AMF COMMUNITY COMPOSITION DATA
# Load data
...
df_fam <- df_asv
df_tax <- df_tax[order(match(rownames(df_tax), colnames(df_fam))),]
colnames(df_fam) <- df_tax[,3]
df_fam <- cbind(sapply(unique(colnames(df_fam)[duplicated(colnames(df_fam))]), function(x) rowSums(df_fam[,grepl(x, colnames(df_fam))])), df_fam[,!duplicated(colnames(df_fam)) & !duplicated(colnames(df_fam), fromLast = TRUE)])
